...
*** TODO Ain't working as of now
** TODO Restrict the number of final suggestions for which we look up MS API
...
** TODO Sliding window
** TODO Deal with the cases where the number of terms in the suggestion and query are different (eg. run-on and split queries).
* Evaluation
